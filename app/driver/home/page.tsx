...
export default async function DriverHomePage() {
    ...
    const overrides = await prisma.dailySalaryOverride.findMany({ where: { driverId: driver.id } });
    ...
    const date = new Date(lastPayout?.paidAt || driver.joiningDate);
        ...
        <main className="max-w-5xl mx-auto px-6 py-8 text-black">
            ...
            <div className="flex gap-3 flex-wrap">
                <a
                    ...
                    className="bg-gray-800 hover:bg-black text-white px-4 py-2 rounded-md text-sm transition"
                >
                    Update Profile
                </a>
                ...
                <a
                    ...
                    className="bg-gray-100 hover:bg-gray-200 text-black px-4 py-2 rounded-md text-sm transition border"
                >
            ...
            <section className="pt-6">
                ...
                <div className="max-h-[300px] overflow-y-auto border border-gray-200 rounded-md bg-white shadow">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {allDaysSinceJoining.map((entry, idx) => (
                                <tr key={idx} className="border-t hover:bg-gray-50">
                                    <td className="px-4 py-2">{entry.date}</td>
                                    <td className="px-4 py-2">₹{entry.amount}</td>
                                    <td className="px-4 py-2">{entry.overridden ? "Adjusted" : "Default"}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
